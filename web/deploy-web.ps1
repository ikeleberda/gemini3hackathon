...
Write-Host "🚀 Deploying web app to Google Cloud Run..." -ForegroundColor Cyan
...
Write-Host "🔍 Checking environment and infrastructure..." -ForegroundColor Cyan
...
if ($BACKEND_URL) {
    Write-Host "✅ Found backend service: $BACKEND_URL" -ForegroundColor Green
} else {
    Write-Host "⚠️  Could not find fractal-backend service. Link it manually after deployment." -ForegroundColor Yellow
}
...
if ($SQL_INSTANCE) {
    Write-Host "✅ Found Cloud SQL instance: $SQL_INSTANCE" -ForegroundColor Green
}
...
if ($null -eq $SQL_INSTANCE) {
    Write-Host "⚠️  Cloud SQL instance 'fractal-db' not found. Using placeholder." -ForegroundColor Yellow
...
# Deploy to Cloud Run
Write-Host "🚀 Deploying to Cloud Run using source code..." -ForegroundColor Cyan
